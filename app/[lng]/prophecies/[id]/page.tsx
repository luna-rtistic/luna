...
export default function ProphecyDetailPage({ params }: { params: { id: string } }) {
  ...
  const lng = typeof urlParams?.lng === 'string' ? urlParams.lng : Array.isArray(urlParams?.lng) ? urlParams.lng[0] : 'en';
      ...
      <div className="max-w-4xl mx-auto" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: '2.5rem', maxWidth: 700, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <FeedCard>
            <HolyCode>
              {prophecy.holy}
              <Translation className="translation" style={{ fontFamily: 'Plus Jakarta Sans, Inter, Segoe UI, sans-serif !important' }}>
                {lng === 'ko' && prophecy.meaning_ko ? prophecy.meaning_ko : prophecy.content}
              </Translation>
            </HolyCode>
            <ReactionBar>
            ...
            {allProphecies.map((p) => (
              <Link key={p.id} href={`/prophecies/${p.id}`} style={{ textDecoration: 'none' }}>
                <FeedCard>
                  ...
                  <FeedHolyCode>
                    {p.holy}
                    <FeedTranslation className="translation">{lng === 'ko' && p.meaning_ko ? p.meaning_ko : p.content}</FeedTranslation>
                  </FeedHolyCode>
                  <FeedReactionBar>
