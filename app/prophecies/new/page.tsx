...
function NewProphecyContent() {
  ...
  const [prophecy, setProphecy] = useState<{ code: string; meaning: string; bible?: string } | null>(null);
  ...
  async function handleNext() {
    ...
    if (error) {
      alert('Failed to save prophecy: ' + error.message);
      return;
    }
    router.push(`/prophecies/${data.id}`);
  }
          ...
          {prophecy && (
            <ProphecyText>
              ...
              <Translation>
                ...
                <TTSButton
                  aria-label="Play TTS"
                  onClick={() => handleTTS(prophecy.meaning)}
                  disabled={ttsLoading}
                  title="Play English interpretation"
                  style={{ marginLeft: 8 }}
                >
                  {ttsLoading ? (
